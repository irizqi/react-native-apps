...
import { Card, CardContent, CardHeader } from "@monoriz/ui/components/card"
import {
    Collapsible,
    CollapsibleContent,
    CollapsibleTrigger,
} from "@monoriz/ui/components/collapsible"
import { ChevronRightIcon, FileIcon, FolderIcon, LucideIcon } from "lucide-react"
        ...
        if (fileItem.type === "folder") {
            ...
            const renderFolderIcon = () => {
                ...
                if (typeof fileItem.icon === 'function') {
                    const IconComponent = fileItem.icon as ComponentType<any>
                ...
                >
                    ...
                    <CollapsibleContent className="mt-1">
                        <div className="flex flex-col">
                            {fileItem.isLoading ? (
                                <div className="text-muted-foreground py-2 text-center text-sm" style={{ paddingLeft: `${(itemLevel + 1) * 16}px` }}>
                                    Loading...
                                </div>
        ...
        <Card className={`mx-auto w-full max-w-[16rem] gap-2 ${className || ''}`} size="sm">
            <CardContent>
                <div className="flex flex-col gap-1">
                    {fileTreeProps?.map((item) => renderItem(item))}
                </div>
            </CardContent>
        </Card>
    )
